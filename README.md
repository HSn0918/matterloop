...
python -m pip install --index-url https://pypi.org/simple \
  "matterloop-presets>=0.2.1,<0.3.0"

# 生产与 CI：固定经过验证的制品版本
python -m pip install --index-url https://pypi.org/simple \
  "matterloop-presets==0.2.1"
```

`v0.2.1` 的 12 个发行包均提供 wheel、sdist 和 Trusted Publishing 发布证明；对应的发布记录见
[GitHub Release](https://github.com/huleidada/matterloop/releases/tag/v0.2.1)。建议在虚拟环境中安装，
...
| Redis | [`matterloop-integration-redis`](https://pypi.org/project/matterloop-integration-redis/) | `matterloop_integration_redis` | Redis 检查点、拉取队列、运行仓储和事件发布 |

例如，只安装 Core 和模型抽象，或在基础装配上增加一种队列集成：

```bash
python -m pip install "matterloop-core>=0.2.1,<0.3.0" \
  "matterloop-models>=0.2.1,<0.3.0"

python -m pip install "matterloop-presets==0.2.1" \
  "matterloop-integration-fastapi==0.2.1" \
  "matterloop-integration-celery==0.2.1"
```

Celery 推送队列与 Redis 拉取队列是两种任务传输方案，按部署方式二选一。Redis 的
`RedisCheckpointStore`、运行仓储和事件发布可以与 Celery 组合；检查点与控制面运行记录职责独立。
...
python -m pip install "matterloop-models[openai]>=0.2.1,<0.3.0"

# MCP SDK 与 OpenTelemetry API
python -m pip install "matterloop-tools[mcp]>=0.2.1,<0.3.0" \
  "matterloop-observability[otel]>=0.2.1,<0.3.0"
```
...
预期输出版本 `0.2.1`，且 `pip check` 报告 `No broken requirements found`。
...
- Redis 检查点没有 TTL、列举、删除或跨 Key 事务；Celery 与 Redis 拉取队列不应叠加消费。
