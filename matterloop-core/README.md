...
恢复不会重置已经消耗的时间、cycle 或 attempt。长组件调用由控制器监督，并按
`heartbeat_interval_seconds` 写入 `LOOP_HEARTBEAT`；取消和总超时会先回收组件协程，再提交终态。
...
`resume()` 只接受 `PAUSED` 和 `BLOCKED`，且 `pending_interaction` 必须先被响应。`recover()` 用于进程
崩溃遗留的活跃检查点：`VERIFYING` 会复用已经保存的 `pending_execution`，不会再次调用 Executor；
`EXECUTING` 没有已保存结果时进入 `BLOCKED / RECOVERY_REQUIRED`，由宿主按 `active_operation_id` 对账，
不会盲目重放。显式 `run_id` 也是提交幂等键：相同请求重复提交返回已有快照，不同请求抛
...
`CheckpointConflictError` 并重新读取；它不能覆盖胜者。`LoopCheckpointCodec` 提供严格的当前
JSON 结构编解码：
...
2. 可选 `CheckpointPreparer` 把本次提交所需的关联信息写入上下文；
3. 用旧 revision CAS 保存检查点；
4. 保存成功后，按 sequence 调用 `EventPublisher`。
...
| `CheckpointPreparer` | `await prepare_checkpoint(context, event_types)` | 可选；仅补充随同本次 CAS 保存的关联信息，不改变业务状态 |
...
| `heartbeat_interval_seconds` | 心跳写入间隔；默认 `5.0` 秒，必须为有限正数 |
| `cancellation_poll_interval_seconds` | 长调用取消检查间隔；默认 `0.1` 秒，必须为有限正数 |

常用方法是 `run()`、`resume()`、`recover()`、`submit_human_response()` 和 `cancel()`。`cancel()` 会在
安全边界或受监督长调用的下一次轮询生效；要在启动前拿到关联 ID，可先调用 `create_run_id()`。
...
`Mapping` 只冻结顶层，持久化时其中的值仍须可 JSON 序列化。
...
- `RetryDecision`：必填 `action: RetryAction`；`delay_seconds: float = 0`，必须为有限非负数。
